...
import src.vue.FenListeEmployes;
import src.vue.FenModifierEmploye;
import src.vue.FenNouvelEmploye;
import src.vue.*;
...
public class Main extends Application{
	...
	static public void supprimerEmploye(Employe e) {
		if (e.getMatricule() != -1 && !getLesEmployes().stream().anyMatch(emp -> emp.getSuperieur() == e.getMatricule())) {
			Donnees.supprimerEmploye(e);
		} else {
			System.err.println("WARNING: Impossible de supprimer le supérieur hierarchique de quelqu'un.");
		}
